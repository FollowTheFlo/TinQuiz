...
const getSparqlFlagList = (countryWDList: string[]) => {
 ...
 FILTER (LANG(?label) = "en")
   ...
   console.log('getSparqlFlagList3', encodeURI(queryCountryList.replace(',))','))')).replace(/%2523/g,'%23'))

     const request$ = ajax(encodeURI(queryCountryList.replace(',))','))')).replace(/%2523/g,'%23'))
     .pipe(
        map(response => {
        ...
        map((results:any[]) => {
         ...
         return results.map( (el:any) => { 
             return { 
                ...
                WdCode: el.wdCode.value.replace("http://www.wikidata.org/entity/",""),
                isSelected: false,
                }
            })
