...
const getWDfromDBP = (elDBP: string) => {
  ...
  const queryEntityWD = `https://dbpedia.org/sparql?query=
    ...
    SELECT ?code WHERE {
      ...
      LIMIT 1&format=json`;
  const request$ = ajax(encodeURI(queryEntityWD).replace(/%2523/g, "%23")).pipe(
    map((response: any) => {
      console.log("DBPEDIA response: ", response);
      //extract code from the wikidata link
      return response.response.results.bindings[0].code.value.replace(
...
export { randomizeChoices, getDBPfromWD, getWDfromDBP, randomIntFromInterval };
