...
import { GO_NEXT_QUESTION, END_QUIZ, START_APP } from "./../constants";
import ActionCreators, { Action } from "./../actions";

export const goNextQuestionEpic: Epic<Action> = (action$, state$) =>
  action$.pipe(
    ofType<Action>(GO_NEXT_QUESTION),
    tap(() => console.log("EPIC - GO_NEXT_QUESTION")),
    map((action: Action) => {
      console.log(
        "EPIC - GO_NEXT_QUESTION Length",
        state$.value.quiz.quiz.questions.length
      );
      console.log(
        "EPIC - GO_NEXT_QUESTION index",
        state$.value.uQuiz.questionIndex
      );
      if (
        state$.value.uQuiz.questionIndex ===
        state$.value.quiz.quiz.questions.length
      ) {
        return ActionCreators.userQuizActions.endQuiz({
          quiz: state$.value.quiz.quiz,
          countryImg: state$.value.quiz.selectedFlag.image,
        });
      }
...
export const endQuizEpic: Epic<Action> = (action$, state$) =>
  action$.pipe(
...
export const startAppEpic: Epic<Action> = (action$, state$) =>
  action$.pipe(
