    ...
    action$.pipe(
        ...
        switchMap((action:Action) => {
            ...
            .pipe(
                catchError( error => {
                   ...
                   return of(ActionCreators.geoActions.showGeoErrorMessage("getUserLatLng Error"));
                 }),
                )
        ...
        switchMap((response:any) => {
            ...
            if(response && response.type && response.type === SHOW_GEO_ERROR) {
                return of(ActionCreators.geoActions.showGeoErrorMessage(response.payload));
            ...
            .pipe(
                catchError( error => {
                   ...
                   return of(ActionCreators.geoActions.showGeoErrorMessage("getPlacefromlatLng Error"));
                 }),
                )
          ...
          //if error was thrown from previous obs, response is type Action
            if(result && result.type && result.type === SHOW_GEO_ERROR) {
             ...
             if(!result.features[0] || !result.features[0].context ) {
              return ActionCreators.geoActions.showGeoErrorMessage('No Results from Mapbox');               
            }
            ...
            return  ActionCreators.geoActions.endLocation({location: {
